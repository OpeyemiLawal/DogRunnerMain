...
# Collectible scenes for AAA-style endless runner
...
var victory_points_required: int = 30
var current_points_collected: int = 0

# Second mission (coins)
var victory_coins_required: int = 30
var current_coins_collected: int = 0
var mission_stage: int = 1  # 1 = collect points, 2 = collect coins
...
const COLLECTIBLE_SPAWN_CHANCE = 0.8  # 80% chance to spawn collectibles per road
...
const MAX_COLLECTIBLES_PER_ROAD = 8
...
# Collectible lane positions - closer to center than obstacles
const COLLECTIBLE_LEFT_OFFSET = -6.0
const COLLECTIBLE_MIDDLE_OFFSET = 0.0
const COLLECTIBLE_RIGHT_OFFSET = 6.0
...
func _ready():
	...
	# Hide ComingSoon screen initially and set up CloseCS button
	if has_node("ComingSoon"):
		$ComingSoon.visible = false
		# Enable ComingSoon screen to process while paused
		$ComingSoon.process_mode = Node.PROCESS_MODE_ALWAYS
		
		# Enable CloseCS button to process while paused
		var close_cs_button = $ComingSoon.get_node_or_null("CloseCS")
		if close_cs_button:
			close_cs_button.process_mode = Node.PROCESS_MODE_ALWAYS
			# Remove white outline
			close_cs_button.focus_mode = Control.FOCUS_NONE
	if has_node("HowToPlay"):
		$HowToPlay.visible = false
		# Enable ComingSoon screen to process while paused
		$HowToPlay.process_mode = Node.PROCESS_MODE_ALWAYS
		
		# Enable CloseCS button to process while paused
		var close_cs_button = $HowToPlay.get_node_or_null("CloseHowToPlay")
		if close_cs_button:
			close_cs_button.process_mode = Node.PROCESS_MODE_ALWAYS
			# Remove white outline
			close_cs_button.focus_mode = Control.FOCUS_NONE
	...
	# Add to world_generator group for collectible access
	add_to_group("world_generator")
...
func _load_obstacle_scenes():
	...
	# Load collectible scenes
	var collectible_files = ["Bone.tscn", "collar2.tscn", "collar.tscn", "FoodCan.tscn", "TennisBall.tscn"]
	
	for file_name in collectible_files:
		var file_path = "res://Scenes/Collectiables/" + file_name
		if ResourceLoader.exists(file_path):
			var scene = load(file_path) as PackedScene
			if scene:
				collectible_scenes.append(scene)
...
func _spawn_road_content(road_chunk: Node3D, road_position: Vector3):
	...
	# Spawn collectibles or coins based on mission stage
	var collectible_roll = rng.randf()
	if collectible_roll < COLLECTIBLE_SPAWN_CHANCE:
		if mission_stage == 1:
			_spawn_collectibles_on_road(road_position)
...
func _spawn_collectibles_on_road(road_position: Vector3):
	if collectible_scenes.is_empty():
		return
		
	var num_collectibles = rng.randi_range(3, MAX_COLLECTIBLES_PER_ROAD)
	
	for i in range(num_collectibles):
		var lane = rng.randi_range(-1, 1)  # Random lane
		var forward_offset = rng.randf_range(10.0, 80.0)
		
		# Use specific collectible lane offsets
		var lane_offset = 0.0
		match lane:
			-1:  # Left lane
				lane_offset = COLLECTIBLE_LEFT_OFFSET
			0:   # Middle lane
				lane_offset = COLLECTIBLE_MIDDLE_OFFSET
			1:   # Right lane
				lane_offset = COLLECTIBLE_RIGHT_OFFSET
		
		# Randomly choose a collectible type
		var collectible_scene_index = rng.randi() % collectible_scenes.size()
		var collectible_scene = collectible_scenes[collectible_scene_index]
		...
		$roadHolder.add_child(collectible_instance)
		collectible_instance.global_position = world_pos
		collectible_instance.global_rotation.y = current_rotation
		
		# Track the collectible
		active_collectibles.append(collectible_instance)

func _spawn_coins_on_road(road_position: Vector3):
	if coin_scenes.is_empty():
		return
		
	var num_coins = rng.randi_range(3, MAX_COLLECTIBLES_PER_ROAD)
...
func update_coin_display(coin_count: int):
	if mission_stage == 2:
		# In second mission, show coins in HUD
		...
		# In first mission, show points
		update_points_display(coin_count)

func update_points_display(points: int):
	if player_hud and player_hud.has_method("set_coin_display"):
		player_hud.set_coin_display(points)
	
	# Update points counter and check for mission progression
	current_points_collected = points
	if mission_stage == 1 and current_points_collected >= victory_points_required and not is_game_paused:
		# Start second mission
		_start_second_mission()

func _start_second_mission():
	mission_stage = 2
	
	# Clear all existing collectibles from the world
	_clear_all_collectibles()
	
	# Reset coin counter
	current_coins_collected = 0
	
	# Update mission UI to show second mission
	_update_mission_ui_for_stage2()
	
	# Force mission UI to be visible and start animation
	missonUI.visible = true
	mission_timer = 0.0
	mission_ui_state = "fading_in"
	mission_ui_alpha = 0.0
...
func _set_mission_ui_alpha(alpha: float):
	...
	# Apply to collectible sprite
	var collectible = missonUI.get_node_or_null("Coin")
	if collectible:
		var collectible_modulate = collectible.modulate
		collectible_modulate.a = alpha
		collectible.modulate = collectible_modulate
...
func _set_mission_ui_scale(scale: Vector2):
	...
	var texture_rect = missonUI.get_node_or_null("TextureRect")
	if texture_rect:
		texture_rect.pivot_offset = texture_rect.size / 20
		texture_rect.scale = scale


func _on_customize_pressed() -> void:
	coming_soon.visible = true
	is_game_paused = true
	get_tree().paused = true
	# Show only DogCustomization label
	var label_container = coming_soon.get_node("Label")
	label_container.get_node("DogCustomization").visible = true
	label_container.get_node("Journey").visible = false
	label_container.get_node("Settings").visible = false
...
func _on_misson_pressed() -> void:
	...
	is_game_paused = true
	get_tree().paused = true
	# Show only Journey label
	var label_container = coming_soon.get_node("Label")
	label_container.get_node("DogCustomization").visible = false
	label_container.get_node("Journey").visible = true
	label_container.get_node("Settings").visible = false
...
func _show_victory_screen():
	print("Victory! Player collected ", current_points_collected, " points!")
...
func _on_settings_pressed() -> void:
	...
	is_game_paused = true
	get_tree().paused = true
	# Show only Settings label
	var label_container = coming_soon.get_node("Label")
	label_container.get_node("DogCustomization").visible = false
	label_container.get_node("Journey").visible = false
	label_container.get_node("Settings").visible = true
...
func _on_close_cs_pressed() -> void:
	print("Close CS")
	# Hide all labels in ComingSoon
	if has_node("ComingSoon"):
		var label_container = $ComingSoon.get_node("Label")
		label_container.get_node("DogCustomization").visible = false
		label_container.get_node("Journey").visible = false
		label_container.get_node("Settings").visible = false
		$ComingSoon.visible = false
		print("ComingSoon screen hidden!")
		# Remove blur effect
		_apply_blur_effect(false)
	else:
		print("ERROR: ComingSoon node not found when hiding!")
	# Resume the game (do this last to ensure everything is cleaned up)
	is_game_paused = false
	get_tree().paused = false
